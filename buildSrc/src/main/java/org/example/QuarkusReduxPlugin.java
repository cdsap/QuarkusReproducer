...
public class QuarkusReduxPlugin implements Plugin<Project> {
    ...
    private void registeringTasks(Project target) {
        ...
        TaskProvider<GenTask> quarkusGenerateCode = tasks.register(Constants.QUARKUS_GENERATE_CODE_TASK_NAME, GenTask.class);
        quarkusGenerateCode.configure(task -> {
            ApplicationModel applicationModel = create(target, LaunchMode.NORMAL);
            task.getAppModel().set(applicationModel);
        });

        TaskProvider<GenTask> quarkusGenerateCodeDev = tasks.register(Constants.QUARKUS_GENERATE_CODE_DEV_TASK_NAME, GenTask.class);
        quarkusGenerateCodeDev.configure(task -> {
            task.dependsOn(quarkusGenerateCode);
            ApplicationModel applicationModel = create(target, LaunchMode.DEVELOPMENT);
            task.getAppModel().set(applicationModel);
        });

        TaskProvider<GenTask> quarkusGenerateCodeTests = tasks.register(Constants.QUARKUS_GENERATE_CODE_TESTS_TASK_NAME, GenTask.class);
        quarkusGenerateCodeTests.configure(task -> {
            ApplicationModel applicationModel = create(target, LaunchMode.TEST);
            task.getAppModel().set(applicationModel);
        });
